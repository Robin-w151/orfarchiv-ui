...
interface Options {
  ...
  viewportInset?: CustomViewportInset;
}
...
export default (target: Element, options: Options): CustomScrollAction[] => {
  ...
  const { scrollMode, block, inline, boundary, skipOverflowHiddenElements, viewportInset } = options;
  ...
  const viewportWidth = windowWithViewport.visualViewport ? windowWithViewport.visualViewport.width : innerWidth;
  const viewportHeight = windowWithViewport.visualViewport ? windowWithViewport.visualViewport.height : innerHeight;
  ...
  for (let index = 0; index < frames.length; index++) {
    ...
    const viewportInsetX = viewportInset?.x ?? 0;
    const viewportInsetY = viewportInset?.y ?? 0;

    // If the element is already visible we can end it here
    // @TODO targetBlock and targetInline should be taken into account to be compliant with https://github.com/w3c/csswg-drafts/pull/1805/files#diff-3c17f0e43c20f8ecf89419d49e7ef5e0R1333
    if (
      scrollMode === 'if-needed' &&
      targetTop >= viewportInsetY &&
      targetLeft >= viewportInsetX &&
      targetBottom <= viewportHeight - viewportInsetY &&
      targetRight <= viewportWidth - viewportInsetX &&
      targetTop >= top &&
      targetBottom <= bottom &&
      targetLeft >= left &&
      targetRight <= right
